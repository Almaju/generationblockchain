...
window.voteForCandidate = function(candidate) {
  ...
  $("#msg").html("Votre vote a été envoyé. Le compteur va se mettre à jour dès que le block sera parvenu à la blockchain. Veuillez patienter.")
  $("#candidate").val("");
  $("#vote-tokens").val("");
...
window.buyTokens = function() {
  ...
  $("#buy-msg").html("L'ordre d'achat a été transmis. Veuillez patienter.");
  Voting.deployed().then(function(contractInstance) {
...
window.lookupVoterInfo = function() {
  ...
  Voting.deployed().then(function(contractInstance) {
    contractInstance.voterDetails.call(address).then(function(v) {
      $("#tokens-bought").html("Total des parts achetées : " + v[0].toString());
      let votesPerCandidate = v[1];
      $("#votes-cast").append("Votes par candidat : <br>");
...
$( document ).ready(function() {
  if (typeof web3 !== 'undefined') {
    ...
    console.warn("No web3 detected. Falling back to " + window.location.hostname + ":8545. You should remove this fallback when you deploy live, as it's inherently insecure. Consider switching to Metamask for development. More info here: http://truffleframework.com/tutorials/truffle-and-metamask");
    // fallback - use your fallback strategy (local node / hosted node + in-dapp id mgmt / fail)
    window.web3 = new Web3(new Web3.providers.HttpProvider("http://" + window.location.hostname + ":8545"));
  }
